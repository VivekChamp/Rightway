...
def calculate_values(accounts, gl_entries_by_account, filters):
	...
	pi_data = dd = frappe.db.sql(f""" 

		select 
			pi.cost_center, 
			sum(pi.grand_total) as pi_total 
		from 
	...
	pi_list = [i['cost_center'] for i in pi_data]
	...
	for d in accounts:
		d.update(init.copy())
		# add opening
		# frappe.msgprint(str(d['account_name']))
		# frappe.msgprint(str(d))

		# frappe.msgprint(str(       pi_list          ))
		# animals.index('dog')
		# frappe.msgprint(str(d['account_name']))

		if str(d['name']) in pi_list:
			index = pi_list.index(d['name'])
			d["expense"] += pi_data[index]['pi_total']

		frappe.msgprint(d["expense"] )
